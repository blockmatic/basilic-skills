---
name: b-use-frontend
description: Build or reshape UI with purpose, existing tokens, accessible interactions, and bounded rendered verification. Use when the user types /b-use-frontend.
disable-model-invocation: true
---

## Purpose and inputs

Build or reshape a user-facing surface. Planning a feature without UI work stays on `/b-plan-feature`. Installing a primitive stays on `/b-use-shadcn`. Do not write PRODUCT.md or DESIGN.md. Do not install animation libraries or design-detector hooks.
...
8. **Optional checklists.** `/b-audit-accessibility` for the Quality overlay (do not invent a WCAG level). `@web-design-guidelines-v1` for code-level interface rules. Playwright E2E only when an existing spec covers the path—it does not replace the screenshot loop.
