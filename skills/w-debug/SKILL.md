...
2. Locate the failing boundary: compiler, types, browser, API, database, dependency, environment, or test. Separate observed facts from hypotheses. Spawn read-only evidence gatherers (log vs recent diff vs owning module) when those sources are independent.
