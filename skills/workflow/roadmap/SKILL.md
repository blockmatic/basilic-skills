...
Canonical horizon: `## Roadmap` in `_first/basilic/PRODUCT.md`. Backlog = GitHub Issues. `__dev/` is scratch.
...
5. **Ask user**: Ask whether to update `_first/basilic/PRODUCT.md` (`## Roadmap`) and/or open GitHub Issues. Plans follow @.cursor/rules/base/general.mdc (References, assumptions, deferrals)
6. **Output**: Show the assessment in chat. **Write** `_first/basilic/PRODUCT.md` (`## Roadmap`) when the user confirms a durable horizon change. Do not leave the roadmap only in chat. Do not write `ROADMAP.md` at repo root. Do not treat `__dev/` as the roadmap.
