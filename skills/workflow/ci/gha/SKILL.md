...
4. Re-run the same local commands the workflow uses. Push only when the user asked to publish. Do not add `gh run watch` to every push.

## Verification

- [ ] The failing check's log is the evidence, not a guess.
- [ ] Local equivalent of the failed job was re-run.
- [ ] No commit or push unless the user requested `/commit` or `/push`.
