...
1. Ensure task-owned changes are committed and validated using the repository's checks. Use [git-commit](../git-commit/SKILL.md) if needed, preserving unrelated work. If this playbook must create a branch first, follow [exec-push](../exec-push/SKILL.md): `git fetch origin` and create the branch from `origin/main`, never a stale local `main`.
