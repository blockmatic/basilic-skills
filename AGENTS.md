...
Explore and review playbooks spawn 2–3 read-only explorers via [w-council](skills/w-council/SKILL.md). Git, TDD, lint, generate, and publish stay single-writer. Explorers return file:line evidence in one wave. New branches: `git fetch origin` then `git switch -c <name> --no-track origin/main` — never a stale local `main`.
